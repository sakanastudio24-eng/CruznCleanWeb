...
- Key promise: `Recent transformations from mobile detailing appointments.`
- Tone: concise captions, no hype copy.

## Quote
- Section purpose: collect custom-detail requests that do not fit the standard package flow.
- Key promise: `Tell us about the vehicle, the condition, and the result you want.`
- Contact sidebar: phone, email, business hours.

## Contact
- Section purpose: handle general questions before booking.
- Key promise: `Use this form for service-area, prep, or scheduling questions.`
- Note: appointments should still route to booking.
...
- Tone: plain-language draft for legal review.
...
## Email Preview
- Internal note: direct URL only for QA and template review.
- Navigation status: intentionally hidden from desktop and mobile menus.
- Messaging: mock transactional previews only; no private provider API calls.

## Styleguide
- Internal note: development-only reference page.
- Purpose: confirm grayscale tokens (`charcoal`, `ink`, `fog`, `canvas`) and typography alignment.

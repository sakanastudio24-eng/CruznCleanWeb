# Cruizn Clean Front-Facing Copy

This is the customer-visible copy register for Cruizn Clean It is organized by page/surface so wording can be reviewed in one place before launch

## Home

### Spring Banner
- Use SPRING20 for 20% off eligible services

### Hero
- Image alt: Cruizn Clean exterior detail finish
- Cruizn Clean
- Memorable detailing without the guesswork
- A darker, cleaner booking experience for Yorba Linda clients who want sharp pricing, polished results, and less friction between quote, booking, and calendar handoff
- Book an Appointment
- (555) 123-4567

### Hero Info Cards
- Location
- Yorba Linda
- Placentia
- Fullerton
- Anaheim
- Brea
- Orange
- La Habra
- Buena Park
- Cypress
- Irvine
- Huntington Beach
- Costa Mesa
- Tustin
- Garden Grove
- Santa Ana
- Availability
- Monday-Saturday 8am - 6pm
- Best Fit
- Maintenance, coatings, and multi-car scheduling

### Quick Detail Finder
- Quick Detail Finder
- Build a cleaner first pass
- Choose a vehicle type, line up the right package, then refine everything else on Services or Booking
- Step 1 • Vehicle Type
- Sedan / Coupe
- Small SUV / Truck
- Large SUV / Truck
- Oversized
- Step 2 • Package
- Mini Detail
- $60
- Maintenance Detail
- $99
- Full Interior
- $179
- Full Exterior
- $279
- The Full Reset
- $399
- Pick a vehicle type first, then choose the package that fits the job
- Build Service Plan
- Need Help First?

### Detail Packages
- Detail Packages
- Choose your service level
- Full service menu
- from $60
- Mini Detail
- Quick exterior wash, vacuum, and light reset for vehicles that just need to look presentable again
- from $99
- Maintenance Detail
- Our most balanced upkeep detail for maintained vehicles that need consistent care
- from $179
- Full Interior
- Deep interior service for fabric, leather, odor, and full touchpoint reset
- from $279
- Full Exterior
- Exterior decontamination, gloss enhancement, and durable protection
- from $399
- The Full Reset
- Full interior and exterior coverage when the vehicle needs the full day reset

### How It Works
- How It Works
- Simple flow, clear results
- Step 1
- Choose Service Plan
- Review packages, correction work, and coatings for each vehicle using the service planner
- Step 2
- Submit Booking
- Send vehicle details, contact info, and service notes in one intake flow
- Step 3
- Make Your Downpayment
- Choose the final appointment time and pay the deposit inside Cal.com after the intake is saved

### Our Work
- Our Work
- Recent transformations
- View full gallery
- Exterior gloss finish
- Exterior reset in natural light
- Rear quarter exterior finish
- Rear profile exterior detail

### Outcome Highlights
- Outcome Highlights
- Results clients can actually see
- Paint Clarity Restored
- Daily-driven paint corrected into a cleaner, sharper finish with stronger gloss and reflection
- Interior Reset
- Cabins reset for cleaner touchpoints, fresher fabric, and a calmer in-car feel
- Protection Layered
- Sealants and coatings added to help defend against road film, dust, and Southern California sun exposure

### Client Feedback
- Client Feedback
- What clients notice after the work is done
- Verified Client
- “The sedan looked sharper the same afternoon, and the booking flow stayed simple from phone to calendar.”
- Jordan R.
- Maintenance Detail
- Verified Client
- “Clear pricing, quick scheduling, and the SUV came back looking noticeably cleaner than expected.”
- Amanda T.
- Full Exterior + 3 Year Ceramic Coating
- Verified Client
- “Setting up two vehicles in one booking felt organized instead of messy.”
- Chris M.
- Two-Vehicle Booking

### Final Step
- Final Step
- Ready to lock in your detail?
- Submit your vehicle and service details first, then finish appointment scheduling in Cal.com with a clean handoff
- Continue to Booking
- Request a Quote
- Intake submitted before scheduling
- Your service selections stay attached to your booking request
- Multi-vehicle workflow ready
- Build and submit multiple cars in one appointment request
- Professional finish, clear pricing
- Package + add-on totals stay visible throughout checkout

## Services

### Page Hero
- Cruizn Clean Services
- Review package and add-on options for Yorba Linda mobile detailing with live size-based pricing and clearer routing for specialty vehicles
- Maintenance Detail is the most balanced option for regularly cared-for vehicles

### Select Your Vehicle
- Select Your Vehicle
- Pick the active vehicle from the dock, then match it to the closest standard category before selecting services
- Vehicle Lookup
- Quick Pick (Dropdown)
- Select from top 50 vehicles
- Type Finder
- Type make or model (e.g. Camry, Model Y)
- This vehicle is not in our standard sizing guide yet Select the closest standard vehicle type, or request a custom quote if the vehicle is modified, lifted, or specialty fitment
- Mismatch check
- If this vehicle has modifications, lift kits, oversized wheels, accessories, or anything else that changes service time, request a custom quote before booking
- Request a Quote
- Review Help + FAQ
- Model-level matching only in v1; when your exact vehicle is not listed, confirm fitment through Quote or FAQ before submitting
- Sedan / Coupe
- Base listed pricing
- Small SUV / Truck
- +20% pricing adjustment
- Large SUV / Truck
- +40% pricing adjustment
- Oversized, lifted, modified, specialty, or unlisted vehicles should get a custom quote before scheduling

### Detail Packages
- Detail Packages
- Mini Detail
- Starts at $60
- About 60-90 mins
- Friendly reset for vehicles that do not need a perfection-level detail
- Foam Bath + Hand Wash
- Wheels + Tires Clean
- Tire shine
- Light Wipe down + Vacuum
- 1 month spray wax
- Maintenance Detail
- Starts at $99
- About 90 mins
- Built for maintained cars that need a dependable inside-out reset
- Designed for maintained cars
- Foam Bath + Hand Wash
- Wheels + Tires Clean
- Scrub/Wipe Down + Vacuum
- Tire Shine
- 3 Months Ceramic Sealant
- Full Interior
- Starts at $179
- About 3 hrs
- Deep interior service for fabric, leather, odor, and full touchpoint reset
- Vacuum
- Upholstery + Carpet Shampoo/Extraction
- Thorough scrub/wipe down of full interior
- Odor Improvement
- Clean Door jambs
- Clean Windows
- Leather conditioner (if applicable)
- Full Exterior
- Starts at $279
- About 3 hrs
- Exterior decontamination, gloss enhancement, and durable protection
- Foam Bath + Hand Wash
- Wheels + Tires Clean
- Clay Bar + Iron Decon
- Light gloss enhancing polish
- 12 months Graphene Ceramic Sealant
- Plastic trim conditioner
- Clean windows
- The Full Reset
- Starts at $399
- About 6-8 hrs
- Full interior and exterior coverage for a complete vehicle reset
- Full Interior
- Full Exterior

### Add-Ons
- Add-Ons
- Standalone premium work for vehicles that need coating, correction, or more specialized protection without forcing a package
- Need a custom setup?

### Paint Correction
- Paint Correction
- 1 Step Paint Correction
- Starts at $449
- Correction service
- Single-step gloss enhancement for defect reduction after prep
- Deep Polish
- Removes 70-80% of Defects
- 2 Step Paint Correction
- Starts at $649
- Correction service
- Cut and polish service for heavier defect reduction after prep
- Cut + Polish
- Removes 80-95% of Defects
- 3 Step Paint Correction
- Starts at $799
- Correction service
- Deep cut, polish, and refining pass for the highest correction tier
- Deep Cut + Polish + Refining Polish
- Removes 95-99% of Defects

### Protection + Coatings
- Protection + Coatings
- 3 Year Ceramic Coating
- Starts at $499
- Requires prep
- Longer-term coating for painted surfaces after required prep
- Covers painted surfaces
- 20% with Paint Correction
- Bundle Savings
- 6 Year Ceramic Coating
- Starts at $799
- Requires prep
- Extended coating package for painted surfaces after prep and inspection
- Covers painted surfaces
- 20% with Paint Correction
- Bundle Savings
- Wheel Face Ceramic Coating
- Starts at $249
- Adds specialty time
- Focused wheel-face protection for the visible outer surface
- Covers visible face of wheel
- Bundle Savings
- Wheel Face + Barrel + Caliper Ceramic Coating
- Starts at $499
- Wheel-off service
- Wheel-off coating service for the full visible wheel and caliper area
- Wheels off car on jack stands
- Covers Visible Face of wheel + Barrel of wheel + Calipers
- Bundle Savings
- Glass Ceramic Coating
- Starts at $200
- No polish needed
- Exterior glass coating when no corrective polish is needed
- Coats Exterior of windows
- Long lasting "invisible" windows
- Bundle Savings
- Glass Ceramic Coating + Polish
- Starts at $279
- Includes polish
- Exterior glass coating with prep polish when the glass needs correction
- Coats Exterior of windows
- Long lasting "invisible" windows
- Includes polish when needed
- Bundle Savings

### Cart / Vehicle Dock
- Vehicle Dock
- Select Vehicle
- Add Another Car
- Maximum 4 vehicles per customer per day
- Pricing Reliability
- Switch cars here, then manage sizing and service selections in the main planner so pricing always updates from one active source of truth
- Cart Summary
- Edit Services
- Book Now
- Book All Vehicles
- Unlock paint coating savings
- Add a paint coating to use the 20% correction savings on paint coating lines
- Add Ceramic Coating
- Complete coating bundle
- Add paint, glass, and wheel coating coverage to apply 20% bundle savings across selected coating lines
- Complete Bundle
- Bundle Savings Applied

### Availability + Limits
- Availability + Limits
- Monday-Saturday availability: 8am - 6pm
- Up to 4 vehicles can be submitted per customer per day
- Same-day requests may require a rush fee

## Booking

### Header
- Book Your Appointment
- Details, schedule, deposit, and confirmation
- Your Details
- Schedule
- Payment

### Vehicle Deck
- Vehicle Deck
- Manage multiple cars in one booking
- Add Vehicle
- Maximum 4 vehicles per customer per day

### Your Details
- Your Details
- Review the active vehicle, complete contact details, and keep your selected services attached before calendar handoff
- Select Your Vehicle
- Match the active vehicle to the closest standard category before selecting services
- Vehicle Lookup
- Quick Pick (Dropdown)
- Select from top 50 vehicles
- Type Finder
- Type make or model (e.g. Camry, Model Y)
- This vehicle is not in our standard sizing guide yet Select the closest standard vehicle type, or request a custom quote if the vehicle is modified, lifted, or specialty fitment
- Mismatch check
- If this vehicle has modifications, lift kits, oversized wheels, accessories, or anything else that changes service time, request a custom quote before booking
- Request a Quote
- Review Help + FAQ
- Sedan / Coupe
- Base listed pricing
- Small SUV / Truck
- +20%
- Large SUV / Truck
- +40%
- Oversized, lifted, modified, specialty, or unlisted vehicles should get a custom quote before scheduling
- Selected Services
- No services selected yet
- Choose a package or add-on on the Services page before scheduling so the estimate and Cal.com metadata stay accurate
- Edit Services
- Booking window: Monday-Saturday 8am - 6pm
- Full Name
- Email
- Phone
- ZIP Code
- Year
- Make
- Model
- Color
- I reviewed the booking terms and policies and agree to be contacted for scheduling updates
- I agree to receive booking confirmations and service-related emails
- SMS confirmations to your phone number
- I agree to receive booking-related SMS confirmations Message/data rates may apply
- Terms
- Privacy
- Help
- Request a Quote
- Continue

### Booking ZIP Validation
- This ZIP is outside the current online booking service area Request a quote so we can review travel and availability
- This ZIP is outside our standard online booking area Request a Quote so travel and availability can be reviewed before scheduling
- Request a Quote

### Schedule
- Schedule
- After intake is saved, the visible schedule content is the inline Cal.com calendar
- Fallback Cal.com link
- Back
- Continue

### Payment
- Payment
- Open secure Stripe Checkout to pay the deposit The deposit is applied toward the final service total
- Estimated total
- Deposit due today
- Secure Stripe Checkout
- Back
- Continue

### Sidebar
- Your Selection
- Active Vehicle
- Booked Vehicles
- Package
- Add-Ons
- Vehicle Subtotal
- Final price confirmed on-site
- Booking Progress
- Step 1 of 3
- Step 2 of 3
- Step 3 of 3
- All vehicles total
- Bundle Savings Applied

## Gallery

### Hero
- Our Work
- Real work from Cruizn Clean service categories, grouped by exterior, interior, wheels, and specialty results

### Filters
- All Work
- Exterior
- Interior
- Wheels + Tires
- Specialty

### Captions
- Exterior gloss finish
- Exterior reset in natural light
- Rear quarter exterior finish
- Rear profile exterior detail
- Body panel finish detail
- Exterior finish pass
- Exterior finish inspection
- Rear exterior reflection
- Interior deep clean reset
- Interior touchpoint cleanup
- Interior finish detail
- Leather seating treatment
- Wheel and tire finish
- Wheel face coating prep
- Tire sidewall detail
- Wheel close-up finish
- Window coating result
- Headlight refinement result
- Headlight restoration detail

## Quote

### Hero
- Request a Custom Quote
- Tell us about the vehicle, the condition, and the result you want We will respond with a tailored estimate for Yorba Linda-area service
- Need sizing or booking help first?

### Form
- Personal Information
- Full Name *
- John Doe
- Email *
- john@example.com
- Phone *
- (555) 123-4567
- Address *
- 123 Main St, City, ST
- Vehicle + Service Request
- Vehicle year, make, model, color, service goals, and any condition notes
- Submit Quote Request
- Sending
- Quote request sent We will reply within 24 hours
- Quote request failed Please retry

### Contact Cards
- Call us
- (555) 123-4567
- Email
- hello@cruiznclean.com
- Business Hours
- Mon-Sat 8:00AM - 6:00PM

## Contact

### Hero
- Contact
- Questions before booking?
- Use this form for general questions about service areas, timing, or prep For appointments, use Book Now
- Yorba Linda, California

### Form
- Full Name
- Email
- Phone
- Optional
- Question
- Send Question
- Sending your message
- Message sent We will respond as soon as possible
- Message failed to send Please try again shortly

## FAQ

### Hero
- Frequently Asked Questions
- Get answers to common questions about our mobile detailing services.

### Service Readiness
- Service Readiness
- Before We Arrive
- Use this quick checklist so your appointment starts on time and your intake details match the service scope.
- Service Area
- Online booking covers selected North OC and nearby 926 ZIPs. If your ZIP is outside the service area, request a quote so travel and availability can be reviewed.
- Ask for a Quote
- 1. Access + Location
- Keep the vehicle in an accessible area with at least 8 feet of clearance from nearby vehicles. Share gate, parking, or arrival notes if needed.
- 2. Vehicle Prep
- Remove loose items, valuables, and personal belongings so interior areas are reachable for cleaning and inspection.
- 3. Handoff
- Be ready for key handoff at the scheduled time and stay clear of the vehicle during service to prevent accidents or injury.
- 4. Final Scope
- Deposit must be confirmed. Final pricing is subject to inspection and can change for incorrect booking details or extended attention.
- Terms & Conditions
- Continue to Booking

### FAQ Filters
- All Questions
- Booking
- Services
- Service Area
- Pricing
- Preparation
- Maintenance

### FAQ Questions + Answers
- How many vehicles can I submit in one day?
- The booking flow supports up to 4 vehicles per customer per day, and actual capacity depends on the length of the selected services.
- What are your booking hours?
- Standard booking hours are Monday through Saturday from 8am to 6pm.
- How long do the main services take?
- Maintenance Detail is roughly 90 minutes. Full Interior or Full Exterior is about 3 hours. A Full Reset usually lands around 6 to 8 hours depending on condition.
- Can I book coatings or paint correction without a detail package?
- Yes. Protection and correction services can be booked on their own, though final prep requirements are confirmed after inspection.
- What service areas can book online?
- Online booking is focused on these service-area cities: Yorba Linda, Placentia, Fullerton, Anaheim, Brea, Orange, La Habra, Buena Park, Cypress, Irvine, Huntington Beach, Costa Mesa, Tustin, Garden Grove, Santa Ana. Standard online ZIP coverage includes 92801-92809, 92821, 92823, 92831-92835, 92840-92846, 92865-92871, 92886-92887, 92602-92620, 92626-92628, 92683-92685. If your ZIP is outside that area, request a quote so travel and availability can be reviewed.
- What if my ZIP code is outside the service area?
- Use Request a Quote instead of standard booking. Outside-area appointments may still be possible, but they need a manual review for travel time, scheduling, and final approval.
- How does pricing change by vehicle size?
- Listed pricing starts with sedans and coupes. Small SUVs and trucks add 20%, large SUVs and trucks add 40%, and vans or very lifted vehicles add 50%.
- How is final pricing confirmed?
- Final pricing is confirmed after inspection and may increase for larger vehicles, excess dirt, pet hair, staining, ride height, or condition-related labor.
- Do you need water or power on-site?
- You do not need to provide water. Share access, parking, gate, or setup notes during booking so the appointment can be approved correctly.
- How should I prepare my vehicle before appointment time?
- Remove loose items and personal belongings before arrival. Full compartments will not be cleaned, and the vehicle should be parked in an accessible location with at least 8 feet of clearance from other vehicles.
- What should I do while the service is being completed?
- Please stay clear of the vehicle during active service to help prevent accidents or injury. Be ready for key handoff at the scheduled time and available for any needed approval.
- Can my quote change after booking?
- Yes. The service quote is subject to change if the vehicle was booked incorrectly, condition is different than described, or the job needs extended attention after inspection.
- Does my deposit need to be confirmed?
- Yes. Deposit confirmation is required before the appointment is treated as locked in.
- What should I know about coatings and correction results?
- Paint correction improves defects but may not remove every scratch or imperfection. Ceramic coating longevity depends on aftercare and environmental exposure.
- How often should I book a maintenance detail?
- Most maintained vehicles benefit from recurring service every 4 to 8 weeks, depending on driving conditions and storage habits.
- What payment methods do you accept?
- Cash, Zelle, Venmo, Cash App, and PayPal are accepted. Debit and credit support may be added later through the booking site.

## Quick Help

### Dialog
- Quick Help
- Top questions from customers
- Service Area
- Standard online booking covers selected North OC and nearby 926 ZIPs. Outside the listed area, use the quote form so travel can be reviewed.
- Ask for a Quote
- Service Requirements
- Loose items removed before arrival
- Accessible parking with 8 feet of clearance
- Stay clear of the vehicle during active service
- Key handoff ready at scheduled time
- Deposit confirmed before service is locked in
- How long does a detail take?
- Maintenance details usually take about 90 minutes. Full interior or exterior services run about 3 hours each, and a full reset can take 6 to 8 hours.
- Do I need to provide water or power?
- No water is required on-site. Share gate, parking, or setup notes during booking so access can be approved correctly.
- How should I prepare before service?
- Remove loose items, park with at least 8 feet of clearance from other vehicles, be ready for key handoff, and stay clear of the vehicle while work is active.
- Can my quote change?
- Yes. The quote can change after inspection if booking details are incorrect or the vehicle needs extended attention.
- What forms of payment do you accept?
- Cash, Zelle, Venmo, Cash App, and PayPal are accepted after service completion. Debit and credit card support is planned next.
- Can I book multiple cars at once?
- Yes. Add vehicles in the dock, select services per vehicle, then book all in one flow.
- What areas can book online?
- Online booking is focused on these service-area cities: Yorba Linda, Placentia, Fullerton, Anaheim, Brea, Orange, La Habra, Buena Park, Cypress, Irvine, Huntington Beach, Costa Mesa, Tustin, Garden Grove, Santa Ana. Standard online ZIP coverage includes 92801-92809, 92821, 92823, 92831-92835, 92840-92846, 92865-92871, 92886-92887, 92602-92620, 92626-92628, 92683-92685. If your ZIP is outside that area, request a quote so travel and availability can be reviewed.
- Need more details? View full FAQ or continue directly to booking.
- View Full FAQ
- Book Now

## Terms of Service

### Intro
- Terms of Service
- These terms outline appointment approval, pricing, deposits, weather rescheduling, and service limits for Cruizn Clean mobile detailing.

### 1. Appointments & Scheduling
- All appointments must be booked in advance.
- Same-day booking requests may require rush fees.
- A confirmation is sent after the booking is approved.
- Please be on time or prepared for arrival to avoid delays or rescheduling.

### 2. Deposits
- A non-refundable deposit of $25-$100 may be required to secure your appointment.
- Any required deposit is applied toward the total service cost.
- No deposit means the booking is not guaranteed.

### 3. Cancellations & Rescheduling
- At least 24 hours notice is required to cancel or reschedule.
- Deposits may be forfeited for late cancellations or no-shows.
- Emergencies are handled case-by-case.

### 4. Pricing
- Listed prices are starting prices for sedans and coupes.
- Small SUVs and trucks add 20%, large SUVs and trucks add 40%, and vans or very lifted vehicles add 50%.
- Pricing can increase based on vehicle size, ride height, excess dirt, grime, stains, pet hair, or condition.
- Final price is confirmed after inspection.

### 5. Vehicle Condition
- Remove personal belongings before the appointment. Full compartments will not be cleaned.
- Cruizn Clean is not responsible for lost or damaged personal items left in the vehicle.

### 6. Service Limitations
- Best-effort results are provided, but not all stains, scratches, or imperfections can be fully removed.
- Paint correction and polishing improve defects but may not eliminate them completely.

### 7. Ceramic Coating / Protection Services
- Proper aftercare is required to maintain durability.
- Longevity claims such as 3-year or 6-year coatings depend on maintenance and environmental conditions.
- There are no guarantees against improper care after service completion.

### 8. Weather Policy (Mobile Services)
- Appointments may be rescheduled due to rain, extreme heat, or unsafe conditions.
- Cruizn Clean will contact you as early as possible if weather requires a change.

### 9. Payment
- Payment is due upon completion unless otherwise agreed.
- Accepted forms: Cash, Zelle, Venmo, Cash App, and PayPal.
- Debit and credit support may be added later through the booking site.
- Checks are not accepted.

### 10. Satisfaction Guarantee
- If you are not satisfied, notify Cruizn Clean before leaving or within 24 hours.
- Reasonable concerns are reviewed and addressed, though additional charges may apply for added scope.

### 11. Liability
- Pre-existing damage is documented before service when visible.
- Cruizn Clean is not responsible for damage caused by faulty parts, aftermarket installs, or prior vehicle conditions.

### 12. Right to Refuse Service
- Service may be refused or rescheduled for unsafe conditions.
- Service may be refused for biohazards, including mold or bodily fluids.
- Service may be refused for disrespectful behavior.
- Service may be refused when the vehicle was described inaccurately before scheduling.

## Privacy Policy

### Intro
- Privacy Policy
- Last updated: February 14, 2026
- Revision note: Initial production-style draft for legal review before public launch.

### 1. Data We Collect
- Booking details (name, email, phone, ZIP code, vehicle details, selected services, intake notes).
- Contact-form details (name, email, phone, message content).
- Communication preferences (email confirmations, SMS preference, SMS consent acknowledgment).
- Operational metadata (submission timestamps and booking identifiers).

### 2. Why We Process Data
- To process booking requests and prepare service operations.
- To send transactional confirmations and owner notifications.
- To respond to customer questions submitted through the contact form.
- To maintain service reliability through operational logging and troubleshooting.

### 3. Service Providers
- We use third-party processors to run business operations. Current processors include:
- We use third-party service providers, including Resend (email delivery) and Cal.com (scheduling), to process bookings and send confirmations. These providers process data solely to deliver our services.
- Resend (transactional email delivery and template management).
- Cal.com (appointment scheduling).
- Hosting provider(s) for frontend and backend infrastructure (for example, Vercel/local infrastructure).

### 4. Retention
- For V1 local storage operations, records are stored in JSON files and retained for operational review, customer support, and business recordkeeping:
- data/bookings.json: booking intake submissions.
- data/contacts.json: contact form submissions.
- data/email_failures.json: failed email send attempts and retry status.
- Formal retention periods will be finalized during legal and compliance review.

### 5. Communications and Consent
- Email confirmations are transactional and sent when selected by the customer.
- SMS is currently preference-and-consent capture only for V1 (no SMS delivery in this phase).
- SMS consent is required before SMS confirmation preference can be submitted.

### 6. Data Rights and Contact
- Rights request and privacy contact channels will be finalized before public launch:
- Privacy email placeholder: privacy@cruiznclean.com
- Business contact placeholder: hello@cruiznclean.com
- This policy draft is not legal advice and is pending attorney review.

## Thank You

### Confirmation
- Booking Flow Complete
- Thanks Your appointment is being confirmed
- Your intake, calendar selection, and Stripe deposit are complete Cruizn Clean will use your saved details to prepare the right service plan for your vehicle
- Watch your inbox
- Cal.com and Cruizn Clean will send booking details and any follow-up instructions
- Prepare the vehicle
- Remove personal items before arrival Final pricing is confirmed after the vehicle condition and size are reviewed

### Service Requirements
- Service Requirements
- Before your appointment
- Loose items and personal belongings must be removed from the vehicle
- Park in an accessible location with at least 8 feet of clearance from other vehicles
- Please stay clear of the vehicle during service to help prevent accidents or injury
- Be ready for key handoff and service at the scheduled time
- Deposit must be confirmed before the appointment is treated as locked in
- The final quote is subject to inspection and may change for incorrect booking details or extended attention
- Review Services
- Call (555) 123-4567

## Global Shell + Footer

### Header
- Cruizn Clean
- Home
- Services
- Gallery
- Quote
- (555) 123-4567
- Book Now
- Open quick help
- Cart Summary

### Mobile Navigation
- Home
- Services
- Gallery
- Quote
- Book

### Footer
- Cruizn Clean
- Mobile detailing for Yorba Linda drivers who want clearer booking, cleaner service menus, and polished results
- Yorba Linda, California
- (555) 123-4567
- Navigate
- Home
- Services
- Privacy Policy
- Gallery
- Terms of Service
- Quote
- Book Now
- Contact
- FAQ
- Service Area
- Yorba Linda, Placentia, Fullerton, Anaheim, Brea, Orange, La Habra, Buena Park, Cypress, Irvine, Huntington Beach, Costa Mesa, Tustin, Garden Grove, Santa Ana
- Business Hours
- Mon-Sat 8:00AM - 6:00PM
- © 2026 Cruizn Clean All rights reserved
- Monday-Saturday booking hours: 8am - 6pm

## Internal Direct-Only Surfaces

### Email Preview
- Email Preview Center
- Customer Confirmation
- Owner Notification
- Email Stack
- Resend
- Template IDs
- Customer Confirmation
- Owner Notification

### Style Guide
- Style Guide
- Grayscale + burgundy tokens, gray-card surfaces, fields, buttons, and internal QA states
